...
class AffineTransform {
    static func calculate(from sourcePoints: [SIMD3<Float>], to targetPoints: [SIMD3<Float>]) -> CGAffineTransform {
        guard sourcePoints.count >= 2, targetPoints.count >= 2 else {
            return .identity
        }
        
        let p1 = SIMD2<Float>(sourcePoints[0].x, sourcePoints[0].z)
        let p2 = SIMD2<Float>(sourcePoints[1].x, sourcePoints[1].z)
        let q1 = SIMD2<Float>(targetPoints[0].x, targetPoints[0].z)
        let q2 = SIMD2<Float>(targetPoints[1].x, targetPoints[1].z)
